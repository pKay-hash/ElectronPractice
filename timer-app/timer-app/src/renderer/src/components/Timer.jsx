...
import alarm from "../assets/sounds/alarm-sound-like-that.mp3"
        ...
        {isEditing ? (
            ...
            <div className='flex justify-center'>
                <div>
                    ...
                    <button className='bg-blue-500 text-stone-200 px-20 py-1 rounded-xl text-xl mt-1 ml-1' onClick={()=>setIsEditing(false)}>&#10004;</button>
                </div>
            </div>
        ) : (
            ...
            <div>
                ...
                <div id="timer-buttons" className= {!isOverlay ? "text-stone-500 flex justify-center bg-black bg-opacity-10 rounded-xl" : "hidden"}>
                    {isActive ? (
                        <>
                            <button onClick={()=>setIsActive(false)} className='start text-5xl text-yellow-500 m-2'>||</button>
                            <button onClick={()=>{setIsActive(false)
                                setHours(0)
                                setMinutes(1)
                                setSeconds(0)
                            }} className='start text-5xl text-red-500 m-2'>&#9632;</button>
                        </>
                    ) : (
                        <>
                            <button className='start text-5xl text-green-500 m-2' onClick={()=>setIsActive(true)}>&#9658;</button>
                            <button className='start text-4xl text-yellow-500 m-2' onClick={()=>setIsEditing(true)}>&#9998;</button>
                        </>
                    )
                }
